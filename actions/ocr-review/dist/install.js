...
async function main() {
  const version = (0, import_shared.getEnv)("OCR_VERSION", false) || "latest";
  const packageSpec = version === "latest" ? "@alibaba-group/open-code-review" : `@alibaba-group/open-code-review@${version}`;
  await (0, import_shared.exec)("npm", ["install", "-g", packageSpec]);
  await (0, import_shared.exec)("ocr", ["version"]);
}
main().catch((error) => {
  console.error(error);
  process.exit(1);
