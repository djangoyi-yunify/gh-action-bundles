...
async function main() {
  ...
  const concurrency = (0, import_shared.getEnv)("CONCURRENCY", false);
  if (concurrency) {
    args.push("--concurrency", concurrency);
  }
  const timeout = (0, import_shared.getEnv)("TIMEOUT", false);
  if (timeout) {
    args.push("--timeout", timeout);
  }
  const rulePath = (0, import_shared.getEnv)("RULE_PATH", false);
  if (rulePath) {
    if (!(0, import_fs.existsSync)(rulePath)) {
      throw new Error(`Rule file not found: ${rulePath}`);
    }
    args.push("--rule", rulePath);
  }
  const exclude = (0, import_shared.getEnv)("EXCLUDE", false);
  if (exclude) {
    args.push("--exclude", exclude);
  }
  const background = (0, import_shared.getEnv)("BACKGROUND", false);
  if (background) {
    args.push("--background", background);
  }
  import_shared.log.info(`Running: ocr ${args.join(" ")}`);
  const { createWriteStream } = await import("fs");
  const stdoutStream = createWriteStream(RESULT_PATH);
  const stderrStream = createWriteStream(STDERR_PATH);
  await new Promise((resolve, reject) => {
    const child = (0, import_child_process.spawn)("ocr", args, {
      stdio: ["ignore", "pipe", "pipe"]
    });
    ...
    child.on("close", (code) => {
      ...
      stderrStream.end();
      if (code !== 0) {
        import_shared.log.warning(`OCR exited with code ${code}`);
      }
      resolve();
    });
  });
